...
const migrations: Migration[] = [
  ...
  },
  {
    ...
    up: [
      ...
      '  name TEXT NOT NULL,',
      '  detail TEXT NULL,',
      '  parent_id UUID NULL REFERENCES group_locations(id) ON DELETE RESTRICT,',
      '  sort_order INT NOT NULL DEFAULT 0,',
      '  created_at TIMESTAMPTZ NOT NULL DEFAULT now()',
      ');',
      'CREATE INDEX IF NOT EXISTS idx_group_locations_group ON group_locations(group_id, sort_order, name);',
      'CREATE INDEX IF NOT EXISTS idx_group_locations_parent ON group_locations(parent_id);',
      'CREATE UNIQUE INDEX IF NOT EXISTS uq_group_locations_siblings ON group_locations(group_id, parent_id, name);',
      '',
      'ALTER TABLE stock_items ADD COLUMN IF NOT EXISTS location_uuid UUID NULL;',
      '',
      "DO $$\nDECLARE g RECORD;\nDECLARE mueble UUID;\nDECLARE freezer UUID;\nDECLARE heladera UUID;\nDECLARE sinubi UUID;\nDECLARE izq UUID;\nDECLARE cen UUID;\nDECLARE der UUID;\nDECLARE izq_a UUID;\nDECLARE izq_b UUID;\nDECLARE cen_a UUID;\nDECLARE cen_b UUID;\nDECLARE der_a UUID;\nDECLARE der_b UUID;\nBEGIN\n  FOR g IN SELECT id FROM family_groups LOOP\n    IF EXISTS (SELECT 1 FROM group_locations WHERE group_id = g.id LIMIT 1) THEN\n      CONTINUE;\n    END IF;\n\n    INSERT INTO group_locations(group_id, name, parent_id, sort_order) VALUES (g.id, 'Mueble', NULL, 0) RETURNING id INTO mueble;\n    INSERT INTO group_locations(group_id, name, parent_id, sort_order) VALUES (g.id, 'Freezer', NULL, 1) RETURNING id INTO freezer;\n    INSERT INTO group_locations(group_id, name, parent_id, sort_order) VALUES (g.id, 'Heladera', NULL, 2) RETURNING id INTO heladera;\n    INSERT INTO group_locations(group_id, name, parent_id, sort_order) VALUES (g.id, 'Sin ubicación', NULL, 99) RETURNING id INTO sinubi;\n\n    INSERT INTO group_locations(group_id, name, parent_id, sort_order) VALUES (g.id, 'Izquierda', mueble, 0) RETURNING id INTO izq;\n    INSERT INTO group_locations(group_id, name, parent_id, sort_order) VALUES (g.id, 'Centro', mueble, 1) RETURNING id INTO cen;\n    INSERT INTO group_locations(group_id, name, parent_id, sort_order) VALUES (g.id, 'Derecha', mueble, 2) RETURNING id INTO der;\n\n    INSERT INTO group_locations(group_id, name, parent_id, sort_order) VALUES (g.id, 'Arriba', izq, 0) RETURNING id INTO izq_a;\n    INSERT INTO group_locations(group_id, name, parent_id, sort_order) VALUES (g.id, 'Abajo', izq, 1) RETURNING id INTO izq_b;\n    INSERT INTO group_locations(group_id, name, parent_id, sort_order) VALUES (g.id, 'Arriba', cen, 0) RETURNING id INTO cen_a;\n    INSERT INTO group_locations(group_id, name, parent_id, sort_order) VALUES (g.id, 'Abajo', cen, 1) RETURNING id INTO cen_b;\n    INSERT INTO group_locations(group_id, name, parent_id, sort_order) VALUES (g.id, 'Arriba', der, 0) RETURNING id INTO der_a;\n    INSERT INTO group_locations(group_id, name, parent_id, sort_order) VALUES (g.id, 'Abajo', der, 1) RETURNING id INTO der_b;\n\n    UPDATE stock_items\n    SET location_uuid = CASE location_id\n      WHEN 'loc_mueble' THEN mueble\n      WHEN 'loc_freezer' THEN freezer\n      WHEN 'loc_heladera' THEN heladera\n      WHEN 'loc_sin_ubicacion' THEN sinubi\n      WHEN 'loc_mueble_izquierda' THEN izq\n      WHEN 'loc_mueble_izquierda_arriba' THEN izq_a\n      WHEN 'loc_mueble_izquierda_abajo' THEN izq_b\n      WHEN 'loc_mueble_centro' THEN cen\n      WHEN 'loc_mueble_centro_arriba' THEN cen_a\n      WHEN 'loc_mueble_centro_abajo' THEN cen_b\n      WHEN 'loc_mueble_derecha' THEN der\n      WHEN 'loc_mueble_derecha_arriba' THEN der_a\n      WHEN 'loc_mueble_derecha_abajo' THEN der_b\n      ELSE sinubi\n    END\n    WHERE group_id = g.id;\n  END LOOP;\nEND $$;",
      '',
      'UPDATE stock_items si',
      'SET location_uuid = gl.id',
      'FROM group_locations gl',
      "WHERE si.location_uuid IS NULL AND gl.group_id = si.group_id AND gl.parent_id IS NULL AND gl.name = 'Sin ubicación';",
      '',
      'ALTER TABLE stock_items ALTER COLUMN location_uuid SET NOT NULL;',
      'ALTER TABLE stock_items DROP CONSTRAINT IF EXISTS stock_items_location_id_fkey;',
      'ALTER TABLE stock_items DROP COLUMN location_id;',
      'ALTER TABLE stock_items RENAME COLUMN location_uuid TO location_id;',
      'ALTER TABLE stock_items ADD CONSTRAINT fk_stock_items_group_location FOREIGN KEY (location_id) REFERENCES group_locations(id) ON DELETE RESTRICT;',
    ].join('\n'),
  },
  {
    version: 7,
    up: [
      ...
      "  HAVING",
      "    COUNT(*) = 13",
      "    AND SUM(CASE WHEN gl.name = 'Mueble' THEN 1 ELSE 0 END) = 1",
      "    AND SUM(CASE WHEN gl.name = 'Freezer' THEN 1 ELSE 0 END) = 1",
      "    AND SUM(CASE WHEN gl.name = 'Heladera' THEN 1 ELSE 0 END) = 1",
      "    AND SUM(CASE WHEN gl.name = 'Sin ubicación' THEN 1 ELSE 0 END) = 1",
      "    AND SUM(CASE WHEN gl.name = 'Izquierda' THEN 1 ELSE 0 END) = 1",
      "    AND SUM(CASE WHEN gl.name = 'Centro' THEN 1 ELSE 0 END) = 1",
      "    AND SUM(CASE WHEN gl.name = 'Derecha' THEN 1 ELSE 0 END) = 1",
      "    AND SUM(CASE WHEN gl.name = 'Arriba' THEN 1 ELSE 0 END) = 3",
      "    AND SUM(CASE WHEN gl.name = 'Abajo' THEN 1 ELSE 0 END) = 3",
      "    AND SUM(CASE WHEN gl.name IN ('Mueble','Freezer','Heladera','Sin ubicación','Izquierda','Centro','Derecha','Arriba','Abajo') THEN 0 ELSE 1 END) = 0",
      "),",
      "safe_groups AS (",
      "  SELECT cg.group_id",
      "  FROM candidate_groups cg",
      "  LEFT JOIN stock_items si ON si.group_id = cg.group_id AND si.deleted_at IS NULL",
      "  WHERE si.id IS NULL",
      ")",
      "DELETE FROM group_locations gl",
      "USING safe_groups sg",
      "WHERE gl.group_id = sg.group_id;",
    ].join('\n'),
  },
  {
    version: 8,
